...
def main():
    ...
    # 2. Analyze Flood Levels
    levels = [2.0, 5.0, 12.0]

    for level in levels:
        print(f"\n--- Flood Level: {level} meters ---")
        submerged = FloodRiskAnalysis.identify_submerged_faces(terrain, level)
        volume = FloodRiskAnalysis.calculate_flood_volume(terrain, level)
