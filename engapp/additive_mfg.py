...
class AdditiveMfg:
    ...
    def detect_overhangs(mesh: TriangleMesh, threshold_angle_deg: float = 45.0, 
                         gravity_dir: Tuple[float,float,float] = (0,0,-1)) -> Tuple[List[int], float]:
        """
        ...
        for i, n in enumerate(face_normals):
            ...
            if dot < limit_cos: # Points mostly "down"
                ...
                total_overhang_area += AdditiveMfg._calculate_triangle_area((v0.x, v0.y, v0.z), (v1.x, v1.y, v1.z), (v2.x, v2.y, v2.z))
    ...
    @staticmethod
    def sample_quaternions(num_samples: int = 100, seed: int = 42) -> List[Tuple[float, float, float, float]]:
        """
    ...
    @staticmethod
    def _rotate_vector_by_quaternion(v: Tuple[float, float, float], 
    ...
    @staticmethod
    def _calculate_triangle_area(v0: Tuple[float,float,float], 
    ...
    @staticmethod
    def find_optimal_rotation(mesh: TriangleMesh, num_samples: int = 100, 
                             threshold_angle_deg: float = 45.0, 
                             gravity_dir: Tuple[float,float,float] = (0,0,-1),
                             alpha: float = 0.0) -> Tuple[Tuple[float,float,float,float], float, float]:
        """
        ...
        for f in faces:
            ...
            centroid = ((v0t[0]+v1t[0]+v2t[0])/3.0, (v0t[1]+v1t[1]+v2t[1])/3.0, (v0t[2]+v1t[2]+v2t[2])/3.0)
        ...
        for q in samples:
            ...
            heights = [v[0]*up_rot[0] + v[1]*up_rot[1] + v[2]*up_rot[2] for v in verts_t]
            min_h = min(heights)
            max_h = max(heights)
            ...
            for i, (area, centroid) in enumerate(face_data):
                ...
                h = (centroid[0]*up_rot[0] + centroid[1]*up_rot[1] + centroid[2]*up_rot[2]) - min_h
                total_moment += area * h
            
            # Normalized Overhang Score (0 to 1)
            overhang_ratio = overhang_area / total_mesh_area if total_mesh_area > 0 else 0
    ...
    @staticmethod
    def _ray_triangle_intersection(ray_origin: Tuple[float,float,float], 
        ...
        h = (ray_dir[1]*edge2[2] - ray_dir[2]*edge2[1],
    ...
    def identify_thin_structures(mesh: TriangleMesh, min_thickness: float) -> List[int]:
        ...
        for i, face_indices in enumerate(faces):
            ...
            eps = 1e-4
            ray_origin = (centroid[0] + ray_dir[0]*eps, centroid[1] + ray_dir[1]*eps, centroid[2] + ray_dir[2]*eps)
            ...
            for j, target_face_indices in enumerate(faces):
                ...
                dist = AdditiveMfg._ray_triangle_intersection(ray_origin, ray_dir, tv0, tv1, tv2)
                if 0 < dist < min_dist:
                    min_dist = dist
    ...
    def filter_printable_mesh(mesh: TriangleMesh, min_thickness: float) -> TriangleMesh:
        ...
        """
        thin_face_indices = set(AdditiveMfg.identify_thin_structures(mesh, min_thickness))
...
def main():
    ...
    parser.add_argument("--optimize", action="store_true", help="Find the optimal rotation of the mesh.")
    parser.add_argument("--samples", type=int, default=100, help="Number of quaternion samples for optimization (default: 100)")
    parser.add_argument("--min-thickness", type=float, help="Minimum wall thickness. If set, will identify and filter thin structures.")
    parser.add_argument("--out-mesh", type=str, help="Path to save the filtered printable mesh.")
    parser.add_argument("--stable", action="store_true", help="Optimize for stability (bottom-heavy) instead of just support count.")
    parser.add_argument("--alpha", type=float, default=0.0, help="Weight for stability optimization (0.0 to 1.0). Default 0.0 (supports only).")
    ...
    if args.min_thickness:
        ...
        if args.out_mesh:
            ...
            printable_mesh = AdditiveMfg.filter_printable_mesh(mesh, args.min_thickness)
            print("Filtered mesh generated.")

    if args.optimize:
        print(f"Detecting initial overhangs for mesh: {args.mesh_file}...")
        initial_overhangs, initial_area = AdditiveMfg.detect_overhangs(mesh, threshold_angle_deg=args.threshold, gravity_dir=tuple(args.gravity))
        print(f"Initial number of faces requiring support: {len(initial_overhangs)}")
        print(f"Initial overhang area: {initial_area:.4f}")

        print(f"Finding optimal rotation using {args.samples} quaternion samples (alpha={alpha})...")
        best_q, min_area, stability_ratio = AdditiveMfg.find_optimal_rotation(mesh, num_samples=args.samples, 
        ...
        if alpha == 0:
            ...
            if reduction > 0:
                print(f"Optimization reduced support area by {reduction:.4f} ({reduction/initial_area*100:.1f}%)")
            else:
                print("Initial orientation is already optimal for supports among samples.")
        else:
        ...
        overhangs, area = AdditiveMfg.detect_overhangs(mesh, threshold_angle_deg=args.threshold, gravity_dir=tuple(args.gravity))
