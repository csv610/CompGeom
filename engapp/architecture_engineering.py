...
try:
    ...
    class TriangleMesh:
        ...
        def from_file(cls, path):
            ...
            return cls([Point3D(0,0,0), Point3D(10,0,0), Point3D(5,10,0)], [[0,1,2]])
...
class ArchitectureEngineering:
    ...
    @staticmethod
    def panelize_surface(mesh: TriangleMesh, panel_width: float, panel_height: float) -> Tuple[int, Tuple[float, float, float]]:
        """
        ...
        if mag < 1e-9: nz_vec = (0, 0, 1)
        else: nz_vec = (nx/mag, ny/mag, nz/mag)

        # 2. Create local coordinate system on the plane
        if abs(nz_vec[0]) < 0.9: ref = (1, 0, 0)
        else: ref = (0, 1, 0)
        ...
        for face in mesh.faces:
            ...
            for i in range(steps + 1):
                for j in range(steps + 1 - i):
                    ...
                    w1 = j / steps
                    w2 = 1.0 - w0 - w1
                    pts_2d.append((p0[0]*w0 + p1[0]*w1 + p2[0]*w2, 
        ...
        for deg in range(0, 91, 10): # Check every 10 degrees
            ...
            for ox in [0, panel_width * 0.25, panel_width * 0.5, panel_width * 0.75]:
                for oy in [0, panel_height * 0.25, panel_height * 0.5, panel_height * 0.75]:
                    # Count unique grid cells covered by points
                    cells = set()
                    for u, v in r_pts:
                        cells.add((math.floor((u - ox) / panel_width), math.floor((v - oy) / panel_height)))
...
def main():
    parser = argparse.ArgumentParser(description="Architecture and Civil Engineering geometry analysis")
    parser.add_argument("--mesh_file", type=str, help="Path to input mesh file (e.g., .stl, .obj)")
    parser.add_argument("--panel_width", type=float, default=1.2, help="Width of the quadrilateral panels")
    parser.add_argument("--panel_height", type=float, default=2.4, help="Height of the quadrilateral panels")
    ...
    print(f"Optimizing panelization for {args.panel_width}m x {args.panel_height}m panels...")
    min_panels, (deg, ox, oy) = arch.panelize_surface(mesh, args.panel_width, args.panel_height)
