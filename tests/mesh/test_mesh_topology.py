...
def test_extract_edges_from_faces():
    from compgeom.mesh.mesh_base import MeshFace

    faces = [MeshFace(0, (0, 1, 2)), MeshFace(1, (0, 1, 3))]
    edges = MeshTopology.extract_edges(faces)

    assert len(edges) == 5
    edge_indices = {e.v_indices for e in edges}
    assert (0, 1) in edge_indices
    assert (1, 2) in edge_indices
    assert (0, 2) in edge_indices


def test_extract_faces_from_cells():
    from compgeom.mesh.mesh_base import MeshCell

    cells = [MeshCell(0, (0, 1, 2, 3)), MeshCell(1, (0, 1, 2, 4))]
    faces = MeshTopology.extract_faces(cells)

    assert len(faces) == 7
    face_indices = {f.v_indices for f in faces}
    assert (0, 1, 2) in face_indices
    assert (0, 1, 3) in face_indices
