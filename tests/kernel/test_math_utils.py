...
def test_orientation():
    ...
    assert orientation(a, b, Point2D(2, 0)) == 0.0
...
def test_orientation_sign():
    ...
    assert orientation_sign(a, b, Point2D(2, 0)) == 0
...
def test_signed_area_twice():
    polygon = [Point2D(0, 0), Point2D(2, 0), Point2D(0, 2)]
    assert signed_area_twice(polygon) == 4.0
    polygon_cw = [Point2D(0, 0), Point2D(0, 2), Point2D(2, 0)]
...
def test_support():
    polygon = [Point2D(0, 0), Point2D(2, 0), Point2D(1, 2)]
    dir = Point2D(0, 1)
    sup = support(polygon, dir)
    assert sup == Point2D(1, 2)
...
def test_triangle_props():
    ...
    assert triangle_circumcenter(a, b, c) == Point2D(1, 1)
...
def test_quad_props():
    ...
    assert is_convex_quad(Point2D(0,0), Point2D(2,0), Point2D(1,1), Point2D(0,2)) is False

def test_segment_props():
    assert segment_midpoint(Point2D(0,0), Point2D(2,2)) == Point2D(1,1)
