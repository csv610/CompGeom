...
def test_closest_pair():
    points = [Point2D(0, 0), Point2D(1, 1), Point2D(0.1, 0.1), Point2D(10, 10)]
    d, (p1, p2) = closest_pair(points)
    assert d == pytest.approx(math.sqrt(0.1**2 + 0.1**2))
    assert (p1 == Point2D(0, 0) and p2 == Point2D(0.1, 0.1)) or (p2 == Point2D(0, 0) and p1 == Point2D(0.1, 0.1))
...
def test_grid_based_massive():
    points = [Point2D(x, y) for x in range(10) for y in range(10)]
...
def test_do_intersect():
    p1, q1 = Point2D(0, 0), Point2D(10, 10)
    p2, q2 = Point2D(0, 10), Point2D(10, 0)
    assert do_intersect(p1, q1, p2, q2) is True
    
    p3, q3 = Point2D(0, 0), Point2D(1, 1)
    p4, q4 = Point2D(2, 2), Point2D(3, 3)
    assert do_intersect(p3, q3, p4, q4) is False

def test_farthest_pair():
    points = [Point2D(0, 0), Point2D(1, 1), Point2D(10, 0), Point2D(0, 10)]
    d, (p1, p2) = farthest_pair(points)
    assert d == pytest.approx(math.sqrt(10**2 + 10**2))
    assert (p1 == Point2D(10, 0) and p2 == Point2D(0, 10)) or (p2 == Point2D(10, 0) and p1 == Point2D(0, 10))
