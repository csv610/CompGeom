from __future__ import annotations

import argparse
from compgeom import simulate_saw_2d

def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="2D Self-Avoiding Walk Simulation.")
    parser.add_argument("--width", type=int, default=100, help="Width of the grid.")
    parser.add_argument("--height", type=int, default=100, help="Height of the grid.")
    parser.add_argument("--start_x", type=int, help="Starting X coordinate.")
    parser.add_argument("--start_y", type=int, help="Starting Y coordinate.")
    args = parser.parse_args(argv)
    
    n, m = args.width, args.height
    sx = args.start_x if args.start_x is not None else n // 2
    sy = args.start_y if args.start_y is not None else m // 2
    
    print(f"--- 2D Self-Avoiding Walk ({n}x{m}) ---")
    print(f"Starting at: ({sx}, {sy})")
    
    res = simulate_saw_2d(n, m, sx, sy)
    
    print(f"\n--- Walk Results ---")
    print(f"Termination: {res['reason']}")
    print(f"Total Steps: {res['steps']}")
    print(f"Unique Cells: {res['unique_cells']}")
    print(f"Final Pos: {res['final_pos']}")
    print(f"Displacement: {res['displacement']:.4f}")
    
    return 0

if __name__ == "__main__":
    raise SystemExit(main())
