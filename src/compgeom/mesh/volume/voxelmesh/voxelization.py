...
class MeshVoxelizer:
    ...
    @staticmethod
    def voxelize(
        mesh: TriMesh, voxel_size: float, fill_interior: bool = True
        ...
        """
        try:
            return MeshVoxelizer.voxelize_openvdb(
                mesh, voxel_size, fill_interior=fill_interior
            )
        except (ImportError, RuntimeError):
            # Fallback to native
            return MeshVoxelizer.voxelize_native(
                mesh, voxel_size, fill_interior=fill_interior
            )

    @staticmethod
    def voxelize_native(
        mesh: TriMesh, voxel_size: float, fill_interior: bool = False
    ) -> Set[Tuple[int, int, int]]:
        """
        ...
        for f in faces:
            ...
            edge3 = math.sqrt(
                (v3.x - v1.x) ** 2 + (v3.y - v1.y) ** 2 + (v3.z - v1.z) ** 2
            )
        ...
        if fill_interior and voxels:
            ...
            for x in range(min_x, max_x + 1):
                for y in range(min_y, max_y + 1):
                    z_boundaries = sorted(
                        [v[2] for v in voxels if v[0] == x and v[1] == y]
                    )
                    if not z_boundaries:
                        continue
    ...
    def voxelize_openvdb(
        ...
        try:
            ...
            raise ImportError(
                "pyopenvdb is not installed. Please install it to use this method."
            )

        v_list = [(v.x, v.y, v.z) for v in mesh.vertices]
        faces = mesh.faces

        grid = vdb.FloatGrid.meshToLevelSet(
            v_list, faces, exBandWidth=bandwidth, inBandWidth=bandwidth
        )
