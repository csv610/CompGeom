...
from compgeom.mesh.surface.processing import flip_normals
...
class TestTriMesh(unittest.TestCase):
    ...
    def test_surface_area(self):
        ...
        self.assertAlmostEqual(MeshGeometry.surface_area(self.mesh), 0.5)

    def test_volume_2d(self):
        # 2D meshes should have 0 volume
        self.assertAlmostEqual(MeshGeometry.volume(self.mesh), 0.0)

    def test_bounding_box(self):
        bbox = MeshGeometry.bounding_box(self.mesh)
        self.assertEqual(bbox, ((0.0, 0.0), (1.0, 1.0)))

    def test_surface_mesh_volume(self):
        # A unit tetrahedron as a surface mesh
        ...
        self.assertAlmostEqual(MeshGeometry.volume(mesh), 1 / 6)
...
class TestTetMesh(unittest.TestCase):
    def test_volume(self):
        # A tetrahedron with volume 1/6: (0,0,0), (1,0,0), (0,1,0), (0,0,1)
        pts = [Point3D(0, 0, 0), Point3D(1, 0, 0), Point3D(0, 1, 0), Point3D(0, 0, 1)]
        cells = [(0, 1, 2, 3)]
...
class TestHexMesh(unittest.TestCase):
    def test_volume(self):
        # A unit cube: (0,0,0) to (1,1,1)
        pts = [Point3D(x, y, z) for z in [0, 1] for y in [0, 1] for x in [0, 1]]
        # Standard VTK ordering for a cube
        cells = [(0, 1, 3, 2, 4, 5, 7, 6)]
        mesh = HexMesh(pts, cells)
        self.assertAlmostEqual(MeshGeometry.volume(mesh), 1.0)
