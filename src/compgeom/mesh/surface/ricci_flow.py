...
class RicciFlow:
    ...
    def solve(self, target_curvatures: Optional[np.ndarray] = None, max_iter: int = 100) -> np.ndarray:
        """
        ...
        res = minimize(objective, self.u, method='L-BFGS-B', options={'maxiter': max_iter})
        self.u = res.x
        return self.u
    ...
    def _compute_curvatures(self, u: np.ndarray) -> Tuple[np.ndarray, Dict[int, List[float]]]:
        ...
        for face in self.mesh.faces:
            ...
            l = [math.exp(u[v_idx[i]]) + math.exp(u[v_idx[(i+1)%3]]) for i in range(3)]
            
            # Law of Cosines for angles
            for i in range(3):
                # Angle at v_i
                a, b, c = l[i], l[(i+1)%3], l[(i+2)%3] # a is opposite to v_i? No.
                # Let's be precise:
                # v0, v1, v2. 
