...
class BinaryImageTriangulation:
    ...
    def detect_edges(self, low_threshold: int = 100, high_threshold: int = 200) -> Tuple[np.ndarray, List[np.ndarray]]:
        ...
        self.contours, self.hierarchy = cv2.findContours(
            self.cleaned_image, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    def compute_triangulations(self) -> Tuple[List[Tuple[Point2D, Point2D, Point2D]], List[Tuple[Point2D, Point2D, Point2D]]]:
