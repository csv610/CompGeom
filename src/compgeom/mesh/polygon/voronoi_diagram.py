...
from compgeom.mesh.surface.trimesh.delaunay_mesh_incremental import (
    IncrementalDelaunayMesher,
    IncrementalTriangle,
)
...
class VoronoiDiagram:
    ...
    def compute(
        self, points: list[Point2D], boundary_type: str = "square"
    ) -> "PolygonMesh":
        """
        ...
        for site, cell in self.cells:
            ...
            for i in range(n):
                ...
                if orientation_sign(p1, p2, site) < 0:
                    print(
                        f"Verification Failed: Site {site} is outside its cell edge ({p1}, {p2})."
                    )
                    return False
                ...
                if orientation_sign(p1, p2, p3) < 0:
                    print(
                        f"Verification Failed: Cell for site {site} is not convex at ({p1}, {p2}, {p3})."
                    )
                    return False
                    ...
                    if d_other < d_site - 1e-7:
                        ...
                        on_boundary = any(
                            abs(
                                orientation_sign(
                                    self.boundary[j],
                                    self.boundary[(j + 1) % len(self.boundary)],
                                    v,
                                )
                            )
                            == 0
                        ...
                        if not on_boundary:
                            print(
                                f"Verification Failed: Vertex {v} of site {site} is closer to {other_site}."
                            )
                            return False
    ...
    def _sort_star(
        self, site: Point2D, star: Iterable[IncrementalTriangle]
    ) -> list[IncrementalTriangle]:
    ...
    def plot(self, width: int = 1000, height: int = 1000, padding: int = 50) -> str:
        ...
        for _, cell in self.cells:
            ...
            svg.append(
                f'<polygon points="{pts_str}" fill="none" stroke="blue" stroke-width="0.5" />'
            )
        ...
        for p in self.points:
            cx, cy = to_c(p)
            svg.append(
                f'<circle cx="{cx:.2f}" cy="{cy:.2f}" r="{site_r}" fill="red" />'
            )
    ...
    @staticmethod
    def get_square_boundary(
        size: float = 100, center: tuple[float, float] = (50, 50)
    ) -> list[Point2D]:
    ...
    def get_circle_boundary(
        radius: float = 50, center: tuple[float, float] = (50, 50), n_segments: int = 64
    ) -> list[Point2D]:
        """Generates a circular boundary polygon."""
        cx, cy = center
        return [
            Point2D(
                cx + radius * math.cos(2 * math.pi * index / n_segments),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of random points (default: 50)",
    )
    parser.add_argument(
        "-b",
        "--boundary",
        ...
        default="square",
        help="Boundary type (default: square)",
    )
    parser.add_argument(
        "-o",
        "--output",
        ...
        nargs="+",
        default=["voronoi.png"],
        help="Output file names (e.g., voronoi.png voronoi.off)",
    )
