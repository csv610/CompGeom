...
def test_shortest_path_in_concave_polygon_routes_through_reflex_vertex():
    ...
    wrapper_path, wrapper_length = polygon_shape.shortest_path(source, target)
    assert wrapper_path == path
    assert wrapper_length == pytest.approx(path_length)


def test_shortest_path_in_polygon_rejects_points_outside_boundary():
    polygon = [Point2D(0, 0), Point2D(4, 0), Point2D(4, 4), Point2D(0, 4)]

    with pytest.raises(ValueError, match="Source point must lie inside or on the boundary"):
        shortest_path_in_polygon(polygon, Point2D(-1, 1), Point2D(1, 1))

    with pytest.raises(ValueError, match="Target point must lie inside or on the boundary"):
