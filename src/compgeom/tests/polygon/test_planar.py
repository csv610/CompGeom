...
def test_face_contains_point():
    ...
    assert face_contains_point(bounded_face, Point2D(0.5, 0.5)) is True
    assert face_contains_point(bounded_face, Point2D(1.5, 0.5)) is False

def test_locate_face():
    poly = [Point2D(0, 0), Point2D(1, 0), Point2D(1, 1), Point2D(0, 1)]
    dcel = build_polygon_dcel(poly)
    face = locate_face(dcel, Point2D(0.5, 0.5))
    assert face.id == 0
    assert face.is_exterior is False
    
    face_ext = locate_face(dcel, Point2D(2, 2))
...
def test_build_polygon_dcel_with_hole():
    ...
    face = locate_face(dcel, Point2D(5, 5))
