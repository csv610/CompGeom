...
def test_polygon_triangulate_and_diagonals():
    polygon = Polygon([Point2D(0, 0), Point2D(2, 0), Point2D(3, 1), Point2D(1, 3), Point2D(0, 2)])
...
def test_visibility_polygon_for_convex_polygon_returns_same_boundary():
    ...
    visible = polygon.visibility_polygon(Point2D(2, 2))

    assert len(visible) == 4
    assert set(visible) == set(polygon.as_list())
    assert set(visibility_polygon(Point2D(2, 2), polygon.as_list())) == set(polygon.as_list())
...
def test_polygon_kernel_for_concave_polygon_returns_smaller_region():
    ...
    assert kernel
    assert len(kernel) == 3
    assert set(kernel) == {Point2D(0, 0), Point2D(4, 0), Point2D(2, 2)}
...
def test_polygon_convexity_reflex_vertices_and_diameter():
    ...
    assert get_reflex_vertices(concave_points) == [Point2D(2, 2)]
    assert concave.reflex_vertices() == [Point2D(2, 2)]
...
def test_polygon_random_generators_return_polygon_compatible_shapes():
    ...
    assert GrahamScan().generate(hull_points) == [Point2D(0, 0), Point2D(1, 0), Point2D(0, 1)]
    assert MonotoneChain().generate(hull_points) == [Point2D(0, 0), Point2D(1, 0), Point2D(0, 1)]
...
def test_convex_hull_quick_hull_matches_other_hull_algorithms():
    ...
    assert ConvexHullGenerator.is_convex_hull(points, [Point2D(0, 0), Point2D(4, 0), Point2D(2, 2), Point2D(0, 4)]) is False
...
def test_visibility_polygon_rejects_outside_viewpoint():
    ...
    with pytest.raises(ValueError):
        polygon.visibility_polygon(Point2D(5, 5))
