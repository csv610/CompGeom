import pytest
from compgeom.kernel import Point, distance
...
def test_point_in_mesh(simple_mesh):
    assert point_in_mesh(simple_mesh, Point(0.5, 0.5)) is True
    assert point_in_mesh(simple_mesh, Point(1.5, 0.5)) is False
...
def test_shortest_path_edges(simple_mesh):
    source = Point(0, 0, id=1)
    target = Point(1, 1, id=3)
...
def test_true_shortest_path_complex():
    ...
    source = Point(4.5, 0.5, id=10)
    target = Point(0.5, 4.5, id=11)
