...
class CirclePackingResult(NamedTuple):
    centers: List[Point2D]
    efficiency: float
    radius: float


def pack_circles(polygon: List[Point2D] | EdgeMesh | Mesh, radius: float) -> CirclePackingResult:
    """
    ...
    """
    if isinstance(polygon, (EdgeMesh, Mesh)):
        polygon = [Point2D(node.point.x, node.point.y) for node in polygon.nodes]
    if not polygon or radius <= 0:
        return CirclePackingResult(centers=[], efficiency=0.0, radius=radius)
...
def optimal_radius(polygon: List[Point2D], num_circles: int, tolerance: float = EPSILON) -> float:
    ...
    for _ in range(50):
        ...
        mid = (low + high) / 2.0
        result = pack_circles(polygon, mid)

        if len(result.centers) >= num_circles:
            best_radius = mid
            low = mid
        else:
            high = mid
