...
def unrotate_point(point, cos_theta, sin_theta):
    return Point(
        point.x * cos_theta - point.y * sin_theta,
        point.x * sin_theta + point.y * cos_theta,
        point.id,
...
def rectangle_corners(min_x, max_x, min_y, max_y, cos_theta, sin_theta):
    ...
    return [unrotate_point(corner, cos_theta, sin_theta) for corner in corners]
...
def minimum_bounding_box(points):
    ...
    for index, start in enumerate(hull):
        ...
        rotated = [rotate_point(point, cos_theta, sin_theta) for point in hull]
        ...
        if best is None or area < best["area"]:
            ...
            best = {
                ...
                "height": height,
                "angle": math.atan2(dy, dx),
                "center": unrotate_point(center_rotated, cos_theta, sin_theta),
