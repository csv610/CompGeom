...
from grid_walks import hilbert_index_to_coords
...
def main():
    ...
    order = args.order if args.order is not None else (int(math.log(min(args.width, args.height), 2)) or 1)
    num_points = 4**order
    print(f"--- 2D Hilbert Curve ---\nOrder: {order} ({2**order}x{2**order})")
    path = [p for i in range(num_points) for p in [hilbert_index_to_coords(i, order)] if p[0] < args.width and p[1] < args.height]
